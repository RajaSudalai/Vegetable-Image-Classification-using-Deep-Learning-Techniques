...
#define file name
modelname   = 'Veg_Classifier_CNN_Tuning_Add_Layer_Conv_Choice_he_normal_regularizer' 
...
def model(datagen, X_train, y_train, X_val, y_val):
    ...
    x = Flatten()(x)
    x = Dense(512, activation='relu',kernel_initializer=he_normal(seed = 33),kernel_regularizer=regularizers.l2({{choice([0.01,0.001])}}))(x)
    x = Dense(128, activation='relu',kernel_initializer=he_normal(seed = 33),kernel_regularizer=regularizers.l2({{choice([0.01,0.001])}}))(x)
